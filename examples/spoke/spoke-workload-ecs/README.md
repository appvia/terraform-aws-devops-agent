...
| <a name="input_aws_region"></a> [aws\_region](#input\_aws\_region) | AWS region for workload account resources. Scopes the permissions boundary to this region — the agent will only read ECS, CloudWatch, ECR, and networking resources here. | `string` | `"eu-west-2"` | no |
...
| <a name="output_permissions_boundary_arn"></a> [permissions\_boundary\_arn](#output\_permissions\_boundary\_arn) | ARN of the ECS permissions boundary policy attached to the cross-account role. |
<!-- END_TF_DOCS -->
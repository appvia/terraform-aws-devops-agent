...
variable "agentspace_region" {
  description = "AWS region where the Agent Space is deployed. Supported regions: us-east-1, us-west-2, ca-central-1, sa-east-1, ap-south-1, ap-southeast-1, ap-southeast-2, ap-northeast-1, eu-central-1, eu-west-1. Note: eu-west-2 has the aidevops service endpoint but the AWS::DevOpsAgent::AgentSpace CloudFormation type is not yet registered there."
  type        = string
  validation {
    condition     = contains(["us-east-1", "us-west-2", "ca-central-1", "sa-east-1", "ap-south-1", "ap-southeast-1", "ap-southeast-2", "ap-northeast-1", "eu-central-1", "eu-west-1"], var.agentspace_region)
    error_message = "Agent Space can only be deployed in us-east-1, us-west-2, ca-central-1, sa-east-1, ap-south-1, ap-southeast-1, ap-southeast-2, ap-northeast-1, eu-central-1, or eu-west-1. Note: eu-west-2 has the service API but the CloudFormation type is not yet registered there."
  }
}
